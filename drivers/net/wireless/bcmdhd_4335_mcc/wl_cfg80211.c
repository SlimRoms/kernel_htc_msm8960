...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0) && (!defined(OLD_CFG_80211))
static s32 wl_cfg80211_bcn_bringup_ap(struct net_device *dev, struct parsed_ies *ies, u32 dev_role, s32 bssidx);
#endif
#endif
static void wl_cfg80211_hotspot_event_process(struct net_device *ndev, const wl_event_msg_t *e, void *data);
	...
	struct key_params *params)
{
    ...
    if ((params->cipher == WLAN_CIPHER_SUITE_WEP40) || (params->cipher == WLAN_CIPHER_SUITE_WEP104))
        err = wldev_iovar_setint(dev, "buf_key_b4_m4", 0);
    else
        err = wldev_iovar_setint(dev, "buf_key_b4_m4", 1);
    if (err) {
        WL_ERR(("Set buf_key_b4_m4 failed,err = %d\n",err));
    }
#endif
	if (mac_addr &&
	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
		...
		if (memcmp(mac, curmacp, ETHER_ADDR_LEN)) {
            ...
            mac_err_cnt++;
            if (mac_err_cnt >= 15) {
                WL_ERR(("Wrong Mac error count is over 15, disconnect\n"));
...
#if defined(CUSTOMER_HW_ONE) && defined(FORCE_MPC)
			if (!strstr(firmware_path, "_p2p"))
#endif
...
void wl_cfg80211_scan_abort(struct wl_priv *wl)
{
	...
	bool aborted, bool fw_abort)
{
	...
	if (fw_abort && !in_atomic() && !check_hang_already(dev)) { 
		wl_cfg80211_scan_abort(wl);
	}
	...
	while (down_interruptible (&tsk->sema) == 0) {
		...
		while ((e = wl_deq_event(wl))) {
			...
			case WLC_E_DEAUTH:
			{
				...
				curbssid = wl_read_prof(wl, cfgdev, WL_PROF_BSSID);
		
				if((wl_get_mode_by_netdev(wl, netdev) == WL_MODE_BSS) &&
...
s32
wl_cfg80211_set_scan_abort(struct net_device *ndev)
{
 	struct wl_priv *wl = wlcfg_drv_priv;
	unsigned long flags;
	s32 ret = BCME_ERROR;
